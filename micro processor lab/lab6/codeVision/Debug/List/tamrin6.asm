...
_0x0:
	.DB  0x41,0x44,0x43,0x20,0x25,0x64,0x20,0x3A
	.DB  0x20,0x25,0x64,0x20,0x6D,0x56,0x0,0x61
	.DB  0x64,0x63,0x20,0x25,0x64,0x3A,0x20,0x25
	.DB  0x64,0x20,0x6D,0x76,0x0,0x20,0x20,0x41
	.DB  0x44,0x43,0x20,0x69,0x73,0x20,0x72,0x65
	.DB  0x61,0x64,0x79,0x0
_0x2000003:
	.DB  0x80,0xC0
...
; 0000 0025 void read_myadcPin (int adcPin){

	.CSEG
_read_myadcPin:
; .FSTART _read_myadcPin
; 0000 0026      ADMUX=adcPin|ADC_VREF_TYPE;
	ST   -Y,R27
	ST   -Y,R26
;	adcPin -> Y+0
	LD   R30,Y
...
; 0000 0027      delay_us(10);
	__DELAY_USB 27
; 0000 0028      ADCSRA=(1<<ADEN) | (1<<ADSC) | (1<<ADATE) | (0<<ADIF) | (1<<ADIE) | (0<<ADPS2) | (1<<ADPS1) | (1<<ADPS0);
	LDI  R30,LOW(235)
	OUT  0x6,R30
; 0000 0029 };
	RJMP _0x2080003
...
;
;void result(void){
; 0000 002B void result(void){
_result:
; .FSTART _result
; 0000 002C     char mydata[];
; 0000 002D     char data[];
; 0000 002E     int devistion=0;
; 0000 002F     devistion=stage2[myadcPin]-stage1[myadcPin];
	ST   -Y,R17
	ST   -Y,R16
;	mydata -> Y+2
;	data -> Y+2
;	devistion -> R16,R17
	...
	SBC  R27,R31
	MOVW R16,R26
; 0000 0030     if(devistion<0)
	TST  R17
	BRPL _0x3
; 0000 0031     devistion=-devistion;
	MOVW R30,R16
	CALL __ANEGW1
	MOVW R16,R30
; 0000 0032     sprintf(data,"ADC %d : %d mV",myadcPin,stage1[myadcPin]);
_0x3:
	MOVW R30,R28
	ADIW R30,2
	ST   -Y,R31
	...
	CALL SUBOPT_0x1
	CALL SUBOPT_0x3
; 0000 0033     if(devistion>((stage1[myadcPin]*5)/100.0))
	CALL SUBOPT_0x1
	LDI  R26,LOW(5)
	LDI  R27,HIGH(5)
	CALL __MULW12U
	...
	RJMP _0x4
; 0000 0034     {
; 0000 0035         sprintf(mydata,"adc %d: %d mv",myadcPin,stage2[myadcPin]);
	MOVW R30,R28
	ADIW R30,2
	ST   -Y,R31
	ST   -Y,R30
...
_0xE:
	RCALL _lcd_gotoxy
; 0000 0040         lcd_puts(data);
	MOVW R26,R28
	ADIW R26,2
	RCALL _lcd_puts
; 0000 0041     }
; 0000 0042     stage1[myadcPin]=stage2[myadcPin];
	MOVW R30,R4
	LDI  R26,LOW(_stage1)
	LDI  R27,HIGH(_stage1)
	LSL  R30
	ROL  R31
	ADD  R30,R26
	ADC  R31,R27
...
; .FSTART _adc_isr
	...
	IN   R30,SREG
	ST   -Y,R30
; 0000 0049 unsigned int adc_data;
; 0000 004A unsigned int volt=0;
; 0000 004B // Read the AD conversion result
...
;	volt -> R18,R19
	__GETWRN 18,19,0
	__INWR 16,17,4
; 0000 004D volt=((adc_data*5)/1023.0)*1000;
	__MULBNWRU 16,17,5
	CALL SUBOPT_0x4
	__GETD1N 0x447FC000
	CALL __DIVF21
...
; 0000 004F stage2[myadcPin]=volt;
	MOVW R30,R4
	LDI  R26,LOW(_stage2)
	LDI  R27,HIGH(_stage2)
	LSL  R30
	ROL  R31
	ADD  R30,R26
	ADC  R31,R27
...
; 0000 0050 result();
	RCALL _result
; 0000 0051 myadcPin=myadcPin+1;
	MOVW R30,R4
	ADIW R30,1
	MOVW R4,R30
; 0000 0052 if(myadcPin == 8)
	LDI  R30,LOW(8)
	LDI  R31,HIGH(8)
	CP   R30,R4
	CPC  R31,R5
...
; 0000 0056  delay_ms(1000);
	...
	LDI  R27,HIGH(1000)
	CALL _delay_ms
; 0000 0057  read_myadcPin(myadcPin);
	MOVW R26,R4
	RCALL _read_myadcPin
; 0000 0058 };
	CALL __LOADLOCR4
	ADIW R28,4
	LD   R30,Y+
...
; 0000 0067 // Function: Bit7=In Bit6=In Bit5=In Bit4=In Bit3=Out Bit2=In Bit1=In Bit0=In
; 0000 0068 DDRB=(0<<DDB7) | (0<<DDB6) | (0<<DDB5) | (0<<DDB4) | (1<<DDB3) | (0<<DDB2) | (0<<DDB1) | (0<<DDB0);
	LDI  R30,LOW(8)
	OUT  0x17,R30
; 0000 0069 // State: Bit7=T Bit6=T Bit5=T Bit4=T Bit3=0 Bit2=T Bit1=T Bit0=T
...
; 0000 007A // Clock value: 125.000 kHz
; 0000 007B // Mode: Fast PWM top=0xFF
; 0000 007C // OC0 output: Non-Inverted PWM
; 0000 007D // Timer Period: 2.048 ms
; 0000 007E // Output Pulse(s):
; 0000 007F // OC0 Period: 2.048 ms Width: 0 us
; 0000 0080 TCCR0=(1<<WGM00) | (1<<COM01) | (0<<COM00) | (1<<WGM01) | (0<<CS02) | (1<<CS01) | (1<<CS00);
	LDI  R30,LOW(107)
	OUT  0x33,R30
...
; 0000 008A ADMUX=ADC_VREF_TYPE;
	LDI  R30,LOW(64)
	OUT  0x7,R30
; 0000 008B ADCSRA=(1<<ADEN) | (0<<ADSC) | (1<<ADATE) | (0<<ADIF) | (1<<ADIE) | (0<<ADPS2) | (1<<ADPS1) | (1<<ADPS0);
	LDI  R30,LOW(171)
	OUT  0x6,R30
...
; 0000 0092  DDRD.3=1;
	SBI  0x11,3
; 0000 0093  lcd_puts("  ADC is ready");
	__POINTW2MN _0x9,0
	RCALL _lcd_puts
; 0000 0094  delay_ms(3000);
	LDI  R26,LOW(3000)
	LDI  R27,HIGH(3000)
	CALL _delay_ms
; 0000 0095  lcd_clear();
	RCALL _lcd_clear
; 0000 0096  read_myadcPin(myadcPin);
	MOVW R26,R4
	RCALL _read_myadcPin
; 0000 0097 while (1)
_0xA:
